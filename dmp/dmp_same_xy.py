...
# model_x = ll.ELLA(d=100, k=100, base_learner=LinearRegression)
# model_y = ll.ELLA(d=100, k=100, base_learner=LinearRegression)
# for i in range(task_num):
#     model_x.fit(X[i], y_x[i], i, theta_x[i]) # 线性回归建模
#     model_y.fit(X[i], y_y[i], i, theta_y[i]) # 线性回归建模
...
# ella_opt = []
# for ii in range(task_num):
#     ella_opt.append(np.hstack((predicted_x[ii],predicted_y[ii])))
...
################### n_bfs = 1 ###########################
    ...
    # !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    # change i in ella_sets[i] to choose use which ella_term in DMP
    y_track_opt, dy_track_opt, ddy_track_opt= dmps.rollout_opt(ella_sets[9])
...
a = []
for i in range(task_num):
    a.append(plt.scatter(task_sets[i][:, 0], task_sets[i][:, 1], marker='x'))
    plt.plot(imitate_trajectory[i][:,0], imitate_trajectory[i][:,1])
plt.legend([a[7]], ['desired path'], loc='lower right')
# for i in range(task_num):
