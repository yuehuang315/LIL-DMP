...
class DMPs(object):
    ...
    def imitate_path(self, y_des, goal_bias, plot=False):
        '''
        Caculate f_target
        ...
        for d in range(self.n_dmps):
            f_target[:, d] = (ddy_des[d] - self.ay[d] *
                              (self.by[d] * (self.goal[d] - y_des[d]) - self.by[d] * (self.goal[d] - self.y0[d]) - dy_des[d]))

        # efficiency generate weight to realized f_target
        w = self.gen_weights(f_target)
    ...
    def rollout(self, timesteps=None, **kwargs):
        ...
        ella_term = np.zeros((timesteps, self.n_dmps))
        x_term = np.zeros(timesteps)

        for t in range(timesteps):
            # run and record timestep
            y_track[t], dy_track[t], ddy_track[t], psi[t], ella_term[t], x_term[t] = self.step(**kwargs)

        return y_track, dy_track, ddy_track, psi, ella_term
    ...
    def step(self, tau=1.0, error=0.0, external_force=None):
        ...
        # Please be attention, psi don't include y informatiom
        # It just decided by canonical system
        psi = self.gen_psi(x)
        # print('psi', len(psi))

        #!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        # Generate ELLA output
        ella_term = []
        # Here, n_dmp = 3
        for d in range(self.n_dmps):
            ella = (x * np.dot(psi, self.w[d])) / np.sum(psi)
            ella_term.append(ella)
        # print("ella_term",ella_term)

        # Here, w[0] is parameter for x, w[1] is parameter for y
        for d in range(self.n_dmps):

            # generate forcing term
            # f = (x * self.gen_front_term(x, d) * (np.dot(psi, self.w[d])) / np.sum(psi))
            f = (self.gen_front_term(x, d) * ella_term[d])

            # DMP acceleration
            self.ddy[d] = (self.ay[d] *
                           (self.by[d] * (self.goal[d] -self.y[d]) -
                            self.by[d] * (self.goal[d] -self.y0[d]) -
                            self.dy[d] / tau) + f) * tau
        ...
        # print("ella_term",ella_term)
        return self.y, self.dy, self.ddy, psi, ella_term, x



    def rollout_opt(self, ella_term_opt, timesteps=None, **kwargs):
        '''
        Run step for timesteps to generate a system trial
        :param timesteps:
        :param kwargs:
        ...
        self.ella_term_opt = ella_term_opt
        # print([ella_term_opt[0,0],ella_term_opt[0,1]])
        # print("ella_term_opt",ella_term_opt)

        # set up tracking vectors
        y_track_opt = np.zeros((timesteps, self.n_dmps))
        dy_track_opt = np.zeros((timesteps, self.n_dmps))
        ddy_track_opt = np.zeros((timesteps, self.n_dmps))

        for t in range(timesteps):
            # run and record timestep
            # ***************************
            step_opt_ella_param = [ella_term_opt[t,0],ella_term_opt[t,1],ella_term_opt[t,2]]
            y_track_opt[t], dy_track_opt[t], ddy_track_opt[t] = self.step_opt(step_opt_ella_param)

        return y_track_opt, dy_track_opt, ddy_track_opt

    def step_opt(self, ella_term_opt):
        '''
        Run the DMP system for a single timestep.
        :param tau: float, scales the timestep, increase tau to make the system execute faster
        :param error: optional system feedback to adjust timestep
        ...
        # Generate ELLA output
        self.ella_term_opt = ella_term_opt
        # print('ella_term_opt', self.ella_term_opt)
        # for d in range(self.n_dmps):
        #     ella = (np.dot(psi, self.w[d])) / np.sum(psi)
        #     ella_term.append(ella)
        ...
        for d in range(self.n_dmps):
            ...
            f_opt = (self.gen_front_term(x, d) * ella_term_opt[d])

            # DMP acceleration
            self.ddy[d] = (self.ay[d] *
                           (self.by[d] * (self.goal[d] -self.y[d]) -
                            self.by[d] * (self.goal[d] -self.y0[d]) -
                            self.dy[d] / tau) + f_opt) * tau
